...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            print(ospj(args.checkpoint_dir, '{:06d}_nets_ema.ckpt'.format(args.resume_iter)))
            self.ckptios = [CheckpointIO(ospj(args.checkpoint_dir, '{:06d}_nets_ema.ckpt'.format(args.resume_iter)), **self.nets_ema)]
    ...
    def synthesis(self, loaders):
        ...
        # Ensure the loader fetches filenames as part of the batch
        for i in tqdm(range(0, len(loaders.val.dataset), args.val_batch_size)):
            # Get the next batch of images and filenames
            inputs_val, fnames = next(fetcher_val)
            # x_src, y_src = inputs_val.x_src, inputs_val.y_src
            
            # Process the batch with filenames
            utils.synthesis_image(nets_ema, args, inputs_val, fnames)
...
"""
    ...
    def synthesis(self, loaders):
        ...
        file_list = sorted(os.listdir(path))  # Ensure filenames are in a consistent order
        # print first 10 samples to make sure they are sorted
        print(file_list[:10])
        for i in tqdm(range(0, len(file_list), args.val_batch_size)):
            # Get the filenames for the current batch
            batch_files = file_list[i:i + args.val_batch_size]
            inputs_val = next(fetcher_val)
            utils.synthesis_image(nets_ema, args, inputs_val, batch_files)
