...
@torch.no_grad()
def synthesis_image(nets, args, inputs, file_list):
    x_src, y_src = inputs.x_src, inputs.y_src
    ...
    # Use filenames instead of step-based numbering
    output_dirs = [os.path.join(args.out_dir, os.path.splitext(file_list[i])[0]) for i in range(len(file_list))]
    [os.makedirs(output_dir, exist_ok=True) for output_dir in output_dirs]

    for psi in [1.0]:
        for idx, output_dir in enumerate(output_dirs):
            save_image(x_src[idx], 1, os.path.join(output_dir, 'Clear.jpg'))

        for i, y_trg in enumerate(y_trg_list):
            for j, z_trg in enumerate(z_trg_list):
                s_trg = nets.mapping_network(z_trg, y_trg)
                x_fake = nets.generator(x_src, s_trg, masks=None)  # Adjust masks as needed
                for idx, output_dir in enumerate(output_dirs):
                    save_image(x_fake[idx], 1, os.path.join(output_dir, f'rain_{j}.jpg'))
"""
@torch.no_grad()
def synthesis_image(nets, args, inputs, file_list):
#     file_list = inputs.fname
    x_src, y_src = inputs.x_src, inputs.y_src
    ...
    # Use filenames from the DataLoader directly instead of generating output directories based on indices
    output_dirs = [os.path.join(args.out_dir, os.path.splitext(file_list[i])[0]) for i in range(len(file_list))]
    [os.makedirs(output_dir, exist_ok=True) for output_dir in output_dirs]
